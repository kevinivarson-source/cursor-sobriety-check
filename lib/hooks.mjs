...
export function isLayerHook(entry) {
  ...
  return (
    ...
    (normalized.includes('cursor-layer') || normalized.includes('cursor-drift-watch'))
  );
}
...
export async function migrateLegacyData() {
  const oldLog = path.join(LEGACY_DATA_DIR, 'events.jsonl');
  if (!existsSync(oldLog) || existsSync(EVENTS_LOG)) return false;
  await mkdir(DATA_DIR, { recursive: true });
  await copyFile(oldLog, EVENTS_LOG);
  return true;
}
